...
def visualize_load_forecast(date, zone, data_path = os.path.join(os.getcwd(), "data"), show_plot = True):
    ...
    if show_plot:
        ...
        for tick in axes[0].xaxis.get_major_ticks():
             tick.label1.set_fontsize(14)  # Directly set the fontsize of tick labels
             tick.label1.set_fontname(font_plot)  # Directly set the fontname of tick labels
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Visualize NYISO forecast and actual load for a given day and zone")
    parser.add_argument("--date", type=str, required=True, help="Date in MM/DD/YYYY format (e.g., 02/14/2024)")
    parser.add_argument("--zone", type=str, required=True, help="NYISO zone ID (e.g., D, G, J)")
    parser.add_argument("--path", type=str, default="./data", help="Path to the data directory (default: ./data)")
    parser.add_argument("--no-show", action="store_true", help="Do not show the plot (only save it)")
    args = parser.parse_args()

    visualize_load_forecast(args.date, args.zone, data_path=args.path, show_plot=not args.no_show)
